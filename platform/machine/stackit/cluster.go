...
`)
	...
	fmt.Printf("Creating server: name: %v, networkId: %v, keypair name: %v, data: %v", *bc.vmname(), *bc.network.NetworkId, *bc.keypair.Name, &configTest)
	instance, err := bc.flight.api.CreateServer(context.TODO(), bc.vmname(), bc.network.NetworkId, bc.keypair.Name, &configTest)
	if err != nil {
