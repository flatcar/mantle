...
func checkAzureNVMe(c cluster.TestCluster) {
	...
	// Check for the presence of the udev rules
	c.MustSSH(m, "test -f /usr/lib/udev/rules.d/80-azure-disk.rules")
	c.MustSSH(m, "test -f /usr/lib/udev/rules.d/10-azure-unmanaged-sriov.rules")

	// Verify that the udev rules are syntactically valid by checking udevadm
	c.MustSSH(m, "sudo udevadm verify /usr/lib/udev/rules.d/80-azure-disk.rules")
	c.MustSSH(m, "sudo udevadm verify /usr/lib/udev/rules.d/10-azure-unmanaged-sriov.rules")
